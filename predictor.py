...
class LotteryPredictor:
    ...
    def _get_ranked_strong_overdue(self) -> List[int]:
        ...
        sorted_overdue = sorted(range(1, 8), key=lambda x: last_seen.get(x, len(draws)), reverse=True)
    ...
    def predict_pattern_based(self, bypass_filters=False) -> Tuple[List[int], List[int]]:
        ...
        draws = self._get_filtered_draws()
        latest_sn = draws[0]['strong_number'] if draws else 1
